...
int operator == (const ROperation & op1, const ROperation & op2) {
	...
	if (op2.mmb2 == NULL && op1.mmb2 != NULL)
		return 0;
	return(((op1.mmb1 == NULL && op2.mmb1 == NULL) || (*(op1.mmb1) == *(op2.mmb1))) && ((op1.mmb2 == NULL && op2.mmb2 == NULL) || (*(op1.mmb2) == *(op2.mmb2))));
}
...
int IsFunction(const char*s, int n) {
	if (CompStr(s, n, "sin") || CompStr(s, n, "cos") || CompStr(s, n, "exp") || CompStr(s, n, "tan") || CompStr(s, n, "log") || CompStr(s, n, "atg") || CompStr(s, n, "abs"))
		return 3;
...
signed char IsFunction(ROperator op) {
	return(op == MEExp || op == Abs || op == MESin || op == MECos || op == Tg || op == MELn || op == Atan || op == Asin || op == Acos || op == Atan || op == MESqrt || op == Opp);
}
...
void IsolateVars(char*&s, int nvar, PRVar*ppvar, int nfunc, PRFunction*ppfunc) // Deletes the old string
{
	...
	for (i = 0; s[i]; i++) {
		...
		if (((j = IsVar(s, i, nvar, ppvar)) > IsFunction(s, i, nfunc, ppfunc)) || (CompStr(s, i, "e") && (j = 1))) {
			InsStr(s, i, '(');
			InsStr(s, i + j + 1, ')');
			i += j + 1;
			continue;
...
ROperation::ROperation(char*sp, int nvar, PRVar*ppvarp, int nfuncp, PRFunction*ppfuncp) {
	...
	if (EqStr(s, "pi") || EqStr(s, "PI") || EqStr(s, "Pi")) {
		op = Num;
		ValC = double (3.141592653589793238462643383279);
		mmb1 = NULL;
		mmb2 = NULL;
		goto fin;
	};
	...
	if (EqStr(s, "e")) {
		op = Num;
		ValC = double (2.71828182845904523536028747135266249);
		mmb1 = NULL;
		mmb2 = NULL;
		goto fin;
	};
	...
	for (k = 0; s[k]; k++) {
		...
		if ( (l = IsFunction(s, k, nfuncp, ppfuncp)) ) {
			if ( l >= IsVar(s, k, nvar, ppvarp) ) {
				i = k + l;
				while (s[i] == ' ')
					i++;
				if (s[i] == '(') {
...
ROperation ROperation::Diff(const RVar& var)const {
	...
	case Atan:
		if (mmb2->op != Juxt)
			...
			return((mmb2->NthMember(1).Diff(var)) * (mmb2->NthMember(2)) - (mmb2->NthMember(2).Diff(var)) * (mmb2->NthMember(1))) / (((mmb2->NthMember(1)) ^ 2) + ((mmb2->NthMember(2)) ^ 2));
	case Asin:
...
char* ValToStr(double x) {
	...
	if (x == (double)3.141592653589793238462643383279L)
		sprintf(s, "pi");
	else if (x == (double)2.718281828459045235360287L)
    	sprintf(s, "e");
    else
		sprintf(s, "%.16G", x);
	return s;
...
void BCDouble(pfoncld*&pf, pfoncld*pf1, pfoncld*pf2, double**&pv, double**pv1, double**pv2, double*&pp, double*pp1, double*pp2, RFunction**&prf, RFunction**prf1, RFunction**prf2, pfoncld f) {
	pfoncld*pf3, *pf4 = pf1;
	long n1, n2;
...
void BCSimple(pfoncld*&pf, pfoncld*pf1, double**&pv, double**pv1, double*&pp, double*pp1, RFunction**&prf, RFunction**prf1, pfoncld f) {
	pfoncld*pf3, *pf4 = pf1;
	long n;
...
void BCFun(pfoncld*&pf, pfoncld*pf1, double**&pv, double**pv1, double*&pp, double*pp1, RFunction**&prf, RFunction**prf1, PRFunction rf) {
	pfoncld*pf3, *pf4 = pf1;
	long n;
	for (n = 0; *pf4 != NULL; pf4++, n++);
	pf = new pfoncld[n + 2];
...
void ROperation::BuildCode() {
	...
	case Var:
		...
		break;
	case Juxt:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &JuxtF);
		break;
	case Add:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &Addition);
		break;
	case Sub:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &Soustraction);
		break;
	case Mult:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &Multiplication);
		break;
	case Div:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &Division);
		break;
	case Pow:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &Puissance);
		break;
	case NthRoot:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &RacineN);
		break;
	case E10:
		BCDouble(pinstr, mmb1->pinstr, mmb2->pinstr, pvals, mmb1->pvals, mmb2->pvals, ppile, mmb1->ppile, mmb2->ppile, pfuncpile, mmb1->pfuncpile, mmb2->pfuncpile, &Puiss10);
		break;
	case Opp:
	...
	case MELn:
		...
		break;
	case MEExp:
		BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals, ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &Exponentielle);
		break;
	case MECos:
	...
	case Tg:
		...
		break;
	case Atan:
		BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals, ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, (mmb2->NMembers() > 1 ? &ArcTangente2 : &ArcTangente));
		break;
	case Asin:
	...
	case Fun:
		...
		break;
	default:
		BCSimple(pinstr, mmb2->pinstr, pvals, mmb2->pvals, ppile, mmb2->ppile, pfuncpile, mmb2->pfuncpile, &FonctionError);
	}
}
