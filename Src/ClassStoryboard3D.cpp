...
__fastcall TStoryBoard3D::TStoryBoard3D(TFmxObject* Owner) : TViewport3D(Owner) {
	...
	Grid3D->Width            = float(GridSize);
	Grid3D->Height           = float(GridSize);
	Grid3D->HitTest          = true;
	Grid3D->Frequency        = float(GridFrequency);
	...
	Camera->Parent           = Boom;
	Camera->Projection       = TProjection::Camera;
	...
	AnimPX->Duration         = float(AnimDuration);
	...
	AnimPY->Duration         = float(AnimDuration);
	...
	AnimPZ->Duration         = float(AnimDuration);
...
void __fastcall TStoryBoard3D::SetGridVisible(bool visible) {
	...
	if (visible)
		...
		Grid3D->Frequency = float(GridSize / 2);
}
...
TVector3D __fastcall TStoryBoard3D::XyToXyz(float X, float Y, TFmxObject *target3D) {
	...
	if (target3D->ClassNameIs(L"TGrid3D")) {
		((TGrid3D*)(target3D))->Context->Pick(xyviewport.X, xyviewport.Y, TProjection::Camera, RayPos, RayDir);
...
TPointF __fastcall TStoryBoard3D::XyzToXy(TVector3D v) {
	...
	p = Grid3D->Context->WorldToScreen(TProjection::Camera , v);
...
TPointF __fastcall TStoryBoard3D::QuerySnapPoint(TPointF point) {
	TPointF r;
	if (point.X >= 0)
		r.X = float(GridFrequency / 2 + (int)(point.X / GridFrequency) * GridFrequency);
	else
		r.X = float(-GridFrequency / 2 + (int)(point.X / GridFrequency) * GridFrequency);
	if (point.Y >= 0)
		r.Y = float(GridFrequency / 2 + (int)(point.Y / GridFrequency) * GridFrequency);
	else
		r.Y = float(-GridFrequency / 2 + (int)(point.Y / GridFrequency) * GridFrequency);
	return r;
}
...
void __fastcall TStoryBoard3D::Expand(bool expcol) {
	...
	for (i = 0; i < VSBListGroup3D.size(); i++)
		VSBListGroup3D.at(i)->IsExpanded = expcol;
}
...
void __fastcall TStoryBoard3D::RemoveFromGroup(TStoryGroup3D *storygroup3D, TStoryImage3D *storyimage3D) {
	...
	if (storygroup3D->Count == 1) {
		...
		storygroup3D->VSGListImage3D[0]->Position->Z = float(0.0);
...
void __fastcall TStoryBoard3D::G3DDragOver(TObject *Sender, const TDragObject &Data, const TPointF &Point, TDragOperation &Operation) {
...
bool __fastcall TStoryImage3D::IsHit(TPointF xy) {
	...
	Context->Pick(xy.X, xy.Y, TProjection::Camera, raypos, raydir);
	if (RayCastPlaneIntersect(raypos, raydir, AbsolutePosition, AbsoluteDirection, p)) {
		if (!IsInGroup) {
			p.X -= Position->X;
			p.Y -= Position->Y;
		}
...
__fastcall TStoryGroup3D::TStoryGroup3D(TFmxObject * Owner, TThumbPanel* thumbpanel, const TVector3D & vector, TStoryBoard3D * storyboard3D) : TImage3D(Owner) {
	...
	AnimY->PropertyName = L"Position.Y";
	AnimY->Enabled = false;
	AnimY->Duration = float(Duration);
	AnimY->StartFromCurrent = true;

	AnimB = new TBitmapAnimation(this);
	AnimB->Parent = this;
	AnimB->PropertyName = L"Bitmap";
	AnimB->Duration = float(Duration);
	...
	Position->Vector    = vector;
	Position->Z         = float(-StepZCollapse);
	RotationAngle->X    = float(0.0);
	RotationAngle->Y    = float(0.0);
	RotationAngle->Z    = float(0.0);
	Width               = thumbpanel->Bitmap->Width / 100;
	...
	ConeR->Position->Y  = 0;
	ConeR->Position->Z  = 0;
	ConeR->RotationAngle->Z = float(90.0);
...
void __fastcall TStoryGroup3D::Add(TStoryImage3D * storyimage3D) {
	...
	if (ExpandMode == EExpandMode::ExpZ) {
		StoryBoard3D->SetAnim(storyimage3D->AnimX, float(0.0));
		StoryBoard3D->SetAnim(storyimage3D->AnimY, float(0.0));
		StoryBoard3D->SetAnim(storyimage3D->AnimZ, float(VSGListImage3D.size() * Step + StepZCollapse));
	}
	else if (ExpandMode == EExpandMode::ExpX) {
		StoryBoard3D->SetAnim(storyimage3D->AnimX, float(VSGListImage3D.size() * StoryBoard3D->GridFrequency));
		StoryBoard3D->SetAnim(storyimage3D->AnimY, float(0.0));
		StoryBoard3D->SetAnim(storyimage3D->AnimZ, float(0.0));
	}
...
void __fastcall TStoryGroup3D::Anime(int i, int j) {
	if (ExpandMode == EExpandMode::ExpZ) {
		StoryBoard3D->SetAnim(VSGListImage3D[i]->AnimX, float(0.0));
		StoryBoard3D->SetAnim(VSGListImage3D[i]->AnimY, float(0.0));
		if ((VSGListImage3D[i]->Position->Z < 0.01) && j > 0) {
			VSGListImage3D[i]->AnimZ->StopAtCurrent();
			VSGListImage3D[i]->Position->Z = float(Step * (j + 2) + StepZCollapse);
			StoryBoard3D->SetAnim(VSGListImage3D[i]->AnimZ, float(Step * (j + 1) + StepZCollapse));
		}
		else {
			StoryBoard3D->SetAnim(VSGListImage3D[i]->AnimZ, float(Step * (j + 1) + StepZCollapse));
		}
...
void __fastcall TStoryGroup3D::AnimeFirst(int i) {
	if (ExpandMode == EExpandMode::ExpZ) {
		...
		if (VSGListImage3D[i]->Position->Z > 2 * Step) {
			VSGListImage3D[i]->Position->Z = float(StepZCollapse);
		}
		else {
			VSGListImage3D[i]->AnimZ->StopAtCurrent();
			VSGListImage3D[i]->Position->Z = VSGListImage3D[i]->Position->Z;
			StoryBoard3D->SetAnim(VSGListImage3D[i]->AnimZ, float(StepZCollapse));
		}
	}
